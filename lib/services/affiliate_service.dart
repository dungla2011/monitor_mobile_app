...
class AffiliateService {
    ...
    try {
      ...
      if (_cachedAffiliateCode != null) {
        ...
        if (timestamp != null) {
          ...
          print(
              '✅ Loaded affiliate code: $_cachedAffiliateCode (saved at $_lastChecked)');
        }
      }
    } catch (e) {
  ...
  static Future<void> _captureInstallReferrer() async {
    try {
      ...
      print(
          'ℹ️ Install referrer capture not implemented yet (use deep links instead)');
    } catch (e) {
  ...
  static Future<void> setAffiliateCode(String code) async {
    try {
      ...
      await prefs.setInt(
          _affiliateTimestampKey, DateTime.now().millisecondsSinceEpoch);
  ...
  static Future<void> parseFromUrl(String url) async {
    try {
      ...
      final affiliateCode =
          params['affiliate'] ?? params['ref'] ?? params['utm_campaign'];
